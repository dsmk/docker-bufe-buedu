...
test_url () {
  ...
  if curl $CURL_ARGS -k -o "$tmp_file" -H "Host:$HOST" "$url" ; then
    ...
    if [ "x$t_retcode" = "x$retcode" ]; then
      ...
      echo "FAIL $label retcode=$t_retcode instead of $retcode"
    ...
    if [ "x$t_upstream" = "x$upstream" ] ; then
      ...
      echo "FAIL $label upstream=$t_upstream instead of $upstream"
      pass_ret_and_upstream=/bin/false
    fi
    if $pass_ret_and_upstream ; then
      echo "PASS $label retcode=$retcode upstream=$upstream"
    fi
 
    # now regexp to be found in the output
    for regexp in "$@" ; do
      if grep -q "$regexp" "$tmp_file" ; then
        echo "PASS $label regexp ($regexp) found"
      else
        echo "FAIL $label regexp ($regexp) NOT found"
      fi 
    done 
  else
    # error occured
    echo "FAIL $label **** could not access URL"
  fi
